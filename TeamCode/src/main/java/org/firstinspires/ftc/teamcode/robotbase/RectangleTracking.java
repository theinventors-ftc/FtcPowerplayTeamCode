...
class RectangleTracking extends OpenCvPipeline
{
    ...
    public Mat processFrame(Mat input)
    {
        ...
        double[] pixel1 = imgHSV.get(
                (int) viewportSize.height/2, (int) viewportSize.width/2);
        double[] pixel2 = imgHSV.get(
                (int) viewportSize.height/2+1, (int) viewportSize.width/2);
        double[] pixel3 = imgHSV.get(
                (int) viewportSize.height/2+1, (int) viewportSize.width/2+1);
        double[] pixel4 = imgHSV.get(
                (int) viewportSize.height/2, (int) viewportSize.width/2+1);

        //Getting Average HSV Values only when Stage=THRESHOLD
        if(colorSet.getAsBoolean()) { //.isPressed()) {
        ...
        Imgproc.threshold(HSLchan0Mat, thresholdMat0, hHSVAverage - rangeAccuracyH,
                255, Imgproc.THRESH_BINARY);
        Imgproc.threshold(HSLchan0Mat, thresholdMat1, hHSVAverage + rangeAccuracyH,
                255, Imgproc.THRESH_BINARY_INV);
        Core.bitwise_and(thresholdMat0,thresholdMat1, thresholdMatH);
        Imgproc.threshold(HSLchan1Mat, thresholdMat0, sHSVAverage - rangeAccuracyS,
                255, Imgproc.THRESH_BINARY);
        Imgproc.threshold(HSLchan1Mat, thresholdMat1, sHSVAverage + rangeAccuracyS,
                255, Imgproc.THRESH_BINARY_INV);
        ...
        for(MatOfPoint c : contoursList) {
            ...
            if(contourArea > Imgproc.contourArea(bigContour))
                bigContour = contoursList.get(contourIdx);
            contourIdx++;
        }
        ...
        Imgproc.drawContours(contoursOnFrameMat, bigContours, -1, new Scalar(0 , 255, 0),
                2, 8);

        //Draw Bounding Rectangle

        Imgproc.rectangle(contoursOnFrameMat, Imgproc.boundingRect(bigContour).tl(),
                Imgproc.boundingRect(bigContour).br(), new Scalar(255,255,0),2);
        ...
        this.createCross(centerCrossPosition, 1, 12, new Scalar(0, 127, 100),
                contoursOnFrameMat);

        rectangleCoordinates[0] = (Imgproc.boundingRect(bigContour).br().x -
                Imgproc.boundingRect(bigContour).tl().x)/2 +
                Imgproc.boundingRect(bigContour).tl().x;
        rectangleCoordinates[1] = (Imgproc.boundingRect(bigContour).br().y -
                Imgproc.boundingRect(bigContour).tl().y)/2 +
                Imgproc.boundingRect(bigContour).tl().y;

        this.createCross(rectangleCoordinates, 1, 8, new Scalar(255, 255, 0),
                contoursOnFrameMat);
    ...
    public void createCross(double[] pos, int thickness, double sizeOfCross, Scalar color,
                            Mat matToRender) {
        Imgproc.line(
                matToRender,
    ...
    public double[] getElementsAnalogCoordinates()
    {
        ...
        rectangleCoordinatesMapped[0] = -(rectangleCoordinates[0] - centerOfScreen[0]) / (
                this.viewportSize.width / 2 + 0.000000000000000000001);
        rectangleCoordinatesMapped[1] = (centerOfScreen[1] - rectangleCoordinates[1]) / (
                this.viewportSize.height / 2 + 0.000000000000000000001);
